...
function Upload() {
  const [imagesMetadata, setImagesMetadata] = useState<{ image: File; title: string, imageURL: string }[]>([]);
  ...
  const handleAddToUploadList = () => {
    ...
    setImagesMetadata([...imagesMetadata, { imageURL, title, image }]);
    setImage(null);
    setTitle("");
  };
    ...
    <div className="flex justify-between w-full max-w-md mt-8 gap-4">
        {
            imagesMetadata.length > 0 && (
                    <button
                        onClick={() => console.log(imagesMetadata)}
